...
class Net_MAC
{
    ...
    {
        // Check for MAC address format XX:XX:XX:XX:XX:XX, XX-XX-XX-XX-XX-XX, or XXXX.XXXX.XXXX or XXxxXXxxXXxx
        $retval = preg_match('/^([0-9a-fA-F]{2}(:|-)){5}[0-9a-fA-F]{2}$|^([0-9a-fA-F]{4}\.){2}[0-9a-fA-F]{4}$|^[0-9a-fA-F]{12}$/', $input);
        return $retval;
    }
     ...
     */
    function setMAC($macaddr, $delimiters = [':', '-', '.'])
    {
    $validMAC = self::check($macaddr, $delimiters);
    if ($validMAC) {
